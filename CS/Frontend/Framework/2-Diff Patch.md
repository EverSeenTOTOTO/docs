...
3.  如果S和T的`tag`相同：
    1.  若S和T的`tag`名称为`text`，比对两者的文本内容，若有变化，修改R的文本内容为T的，跳转至子步骤4；
...
这样所有动作的代价为1，通过比较序列的长度获取代价最小的编辑序列。算法的实现不是我们的重点，这里不再贴出。算法执行后，输出一个待办列表`editions`，其中，`insert`和`delete`最终转为一个同名Patch动作：
