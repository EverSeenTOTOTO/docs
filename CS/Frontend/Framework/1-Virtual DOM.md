...
模式匹配的概念大家应该都很熟悉了，编程语言中常用的`switch/case`便是一种形式的“匹配-分派”过程。如果你仔细观察，会发现这个过程通常有两个特点：“<Notation type="box">从特殊到一般</Notation>”、“<Notation type="box">嵌套使用以简化代码逻辑</Notation>”。
...
“从特殊到一般”还意味着将任务一步步划分并解决，而不是原地空转。也即“递归下降”中的“下降”。体现在编译领域，有点类似文法中终结符和非终结符的区别，不妨想想我们如何消除左递归：

```
A -> Aa | b
```

如果直接按递归下降的思路设计程序，明眼人都能看出来程序会空转：
...
每次`A`或者`A'`执行，都会因为`match b`或者`match a`而“吃掉”输入中的一个字符，这使我们接下来要处理的问题变得更小，直至解决问题。
...
此时，`vnode.type`就成了某种意义上的“终结符”，每次消耗掉一个VNode，问题不断缩小。再通过彼此之间递归嵌套调用，我们在VDOM树上一点点“下钻”并最终走到叶子节点，整棵树处理完毕。
