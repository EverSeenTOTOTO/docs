...
4.  实现协程[^2]
    ...
    为了实现这个目标，我们传入的`yield`应该是一个来自外部的延续，这样调用`yield i`会直接跳过后续的执行，并将程序控制权传到外界。但跳过了后续迭代又如何实现遍历呢？我们可以不仅仅是传递`i`，而同时<Notation>将`gen`内部的“当前延续”也传递出去</Notation>：
...
如果不是使用求值器，而是常规的编译到中间码乃至机器码的流程，对延续的处理要困难得多，我在这里[^5]看到过一些内容，但没有实践。
